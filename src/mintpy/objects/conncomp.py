...
def label_conn_comp(mask, min_area=2.5e3, min_area_ratio=3e-3, erosion_size=5, print_msg=False):
    ...
    if num_label > 1:
        if min_area > label_img.size * min_area_ratio:
            vprint(f'set minimum region size from {min_area} to {label_img.size*min_area_ratio} (image_size * {min_area_ratio*1e2}%)')
